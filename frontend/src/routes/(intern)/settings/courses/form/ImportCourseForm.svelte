<script>
  ...
  import { courses_upload_schema, universities_upload_schema } from '$root/lib/schema';
  ...
  const { form, errors, enhance, reset } = superForm(data, {
    ...
    clearOnSubmit: 'none',
    resetForm: true,
    validators: zod(courses_upload_schema),
...
<dialog bind:this={dialog}>
  <div class="headline dialog-header border-bottom">
    <h2 class="m-0">Studiengänge importieren</h2>
    <button class="btn-close" type="button" aria-label="Close" on:click={() => dialog_close()} />
  </div>

  <form action="?/importCourse" method="POST" enctype="multipart/form-data" use:enhance>
    <!-- Body -->
    <div class="body p-3">
      <div class="row mb-3">
        <div class="col-12 mb-2">
          <p class="text-secondary">
            Bitte lade eine JSON-Datei im richtigen Format hoch. Lade <a href="/examples/example_import_course.json" target="_blank">hier</a> eine Beispiel Datei herunter.
          </p>
        </div>
        <div class="col-12">
