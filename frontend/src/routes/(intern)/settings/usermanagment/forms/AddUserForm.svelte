<script>
  ...
  import { add_user_schema, user_schema } from '$root/lib/schema';
  ...
  const { form, errors, enhance, reset } = superForm(data, {
    ...
    clearOnSubmit: 'none',
    resetForm: false,
    validators: zod(add_user_schema),
...
<dialog bind:this={dialog}>
  <div class="headline dialog-header border-bottom">
    <h2 class="m-0">Benutzer hinzufügen</h2>
  ...
  <form action="?/addUser" method="POST" use:enhance>
    ...
    <div class="body p-3">
      ...
      <div class="row mb-3">
        <div class="col-md-3">
          <label for="name" class="col-form-label">Wiederhole Passwort</label>
        </div>
        <div class="col">
